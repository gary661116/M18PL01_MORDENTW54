...
<form name="form1" id="form1" action="" method="post">
  ...
  <header class="table-head form-inline">
      ...
      <label>類別</label>
      <select id="txt_inside" name="txt_inside">
          <option value="">全部</option>
          <option value="Y">站內</option>
          <option value="N">站外</option>
      </select>      
      ...
      @if ($row_count > 0)
          @for ($i = $min_row - 1; $i < $max_row; $i++)
             <?php
              ...
              if ($info[$i]->is_inside == "Y")
              {
                  $str_inside = "站內";
                  $inside_class = "label-success";
              }
              else
              {
                  $str_inside = "站外";
              ...
              ?>
                  <tr>
                      ...
                      <td>{{ $cate_name }}</td>
                      <td>{{ $sort }}</td>
                      <td><span class="label {{ $inside_class }}">{{ $str_inside }}</span></td>                      
