...
<form name="form1" id="form1" action="" method="post">
  ...
  <header class="table-head form-inline">
      ...
      <label>起</label>
      <input type="date" id="txt_start_date" name="txt_start_date" value="{{ $txt_start_date }}" max="9999-12-31">
      ~
      <label>迄</label>
      <input type="date" id="txt_end_date" name="txt_end_date" value="{{ $txt_end_date }}" max="9999-12-31">
      <label>分類</label>
  ...
  <table class="table-list table-hover table-striped">
      ...
      <tr>
          ...
          <th>
              ...
              <button class="th-sort-toggle {{ $class_date }}" name="btn_date" id="btn_date" onclick="sort('n_date');">日期</button>
          </th>
          ...
          <th>
              {{-- 點選排序功能 (點一下遞增 asc, 點兩下遞減 desc) --}}
              <button class="th-sort-toggle {{ $class_index }}" name="btn_index" id="btn_index" onclick="sort('is_index');">顯示於最新專案區</button>
          </th>
      ...
      @if ($row_count > 0)
          @for ($i = $min_row - 1; $i < $max_row; $i++)
             <?php
              ...
              $c_date = $info[$i]->c_date;
              $c_sort = $info[$i]->sort;
              $cate_name = $info[$i]->cate_name;
              $is_index = $info[$i]->is_index;
              if ($info[$i]->status == "Y")
              {
                  $show = "發佈";
